<script>
	...
	import { faX, faNavicon, faEnvelope } from '@fortawesome/free-solid-svg-icons';
...
<header class="text-white flex items-center justify-between p-2 w-full">
	...
	<nav class="hidden sm:flex items-center justify-center ">
		<ul class="flex items-center justify-center">
			<li class="mx-2 flex justify-center items-center flex-col" class:active={$page.url.pathname === '/'}>
				<a on:click={EffectNavButtons} class="LINK BTT hover:text-blue-400" href="/">Home</a>
			</li>

			<li class="mx-2 flex justify-center items-center flex-col" class:active={$page.url.pathname === '/about'}>
				<a on:click={EffectNavButtons} class="LINK BTT hover:text-blue-400" href="/about">About</a>

			</li>
			<li class="mx-2 flex justify-center items-center flex-col" class:active={$page.url.pathname === '/contact'}>
				<a on:click={EffectNavButtons} class="LINK BTT hover:text-blue-400" href="/contact">contact</a>
			</li>
		</ul>
	</nav>
	...
	<nav class="{(ShowNav) ? "showFromRight" : "-translate-x-full"} fixed w-screen h-screen top-0 left-0 bg-white text-black flex flex-col justify-center items-center sm:hidden">
		<ul class="flex flex-col items-center justify-center">
			<li on:click={Navigate_} group={LinkButtons} class="mx-2 my-4 flex justify-center items-center flex-col" class:active={$page.url.pathname === '/'}>
				<a class="LINK BTT hover:text-blue-400" href="/">Home</a>
			</li>
			<li on:click={Navigate_} group={LinkButtons} class="mx-2 my-4 flex justify-center items-center flex-col" class:active={$page.url.pathname === '/about'}>
				<a class="LINK BTT hover:text-blue-400" href="/about">About</a>

			</li>
			<li on:click={Navigate_} group={LinkButtons} class="mx-2 my-4 flex justify-center items-center flex-col" class:active={$page.url.pathname === '/contact'}>
